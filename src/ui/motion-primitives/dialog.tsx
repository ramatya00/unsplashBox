...
function Dialog({ children, variants = defaultVariants, transition = defaultTransition, defaultOpen, onOpenChange, open }: DialogProps) {
...
function DialogPortal({ children, container = typeof window !== "undefined" ? document.body : null }: DialogPortalProps) {
...
function DialogClose({ className, children, disabled }: DialogCloseProps) {
	...
	return (
		<button onClick={() => context.setIsOpen(false)} type="button" aria-label="Close dialog" className={className} disabled={disabled}>
			{children}
